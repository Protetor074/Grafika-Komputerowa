...
def z1(tab,tabToNormal,n):
    ...
    for i in range(int(n/2)):
        ...
        #Pierwsza połowa przemnożona przez -1 dla normalneji i tosamo przy rysowaniu normalnych
        for j in range(n):
            # if(colmod):
            #     glColor3f(1, 0.6, 0.3)
            # else:
...
def printVectorNormal(tab,tabToNormal,n):
    for i in range(int(n / 2)):
        for j in range(n):

            glBegin(GL_LINES)
            glVertex3f(tab[i][j][0], tab[i][j][1], tab[i][j][2])
            glVertex3f(tab[i][j][0] + tabToNormal[i][j][0], tab[i][j][1] + tabToNormal[i][j][1],
                       tab[i][j][2] + tabToNormal[i][j][2])
            glEnd()
...
def render(time,tab,tabToNormal,n):
    ...
    printVectorNormal(tab,tabToNormal,n)
